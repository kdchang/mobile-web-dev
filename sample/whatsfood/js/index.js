...
var getDetail = function(){
    $(document).on('pagebeforeshow', '#dining-info-page', function(){
        ...
        $.getJSON("http://query.yahooapis.com/v1/public/yql?q=select * from html where url='http://www.ipeen.com.tw" + localStorage.getItem('data-link') + "'" + "&format=json&diagnostics=true&callback=?", function(data) {
            ...
            $('#dining-content').html('<dvi>' + '<h3>' + shop + '<br></h3><div class="fb-like" data-href="http://kdchang.cc/mobile-web-dev/sample/whatsfood/#dining-info-page" data-width="350" data-show-faces="true" data-send="true"></div></div>' + '<h4>營業時間：</h4>' + '<p>' + time + '</p>' + '<h4>商家地址：</h4>' + '<p>' + address + '</p>' + '<a href="tel:' + tel + '"><button class="success" type="submit">電話訂位</button></a>' + '</div>');
        });
    });
};
